...
const CameraFeed = () => {
    ...
    const processFrame = useCallback(async () => {
        ...
        // Throttle processing for real-time performance
        const now = Date.now();
        const active = activeModels[0];
        const minInterval = active?.key === 'hand-gesture-recognition' ? 500 : 1000; // 2 FPS for gestures, 1 FPS for license plates
        ...
        try {
            ...
            // Decide based on active model
            const blobForUpload = blob;
            const file = new File([blobForUpload], 'frame.jpg', { type: 'image/jpeg' });

            if (active && active.key === 'hand-gesture-recognition') {
                const resp = await visionAPI.detectGesture(file);
                if (resp?.success) {
                    const bb = resp.detection?.boundingBoxes?.[0];
                    const result = {
                        label: resp.detection?.gestureLabel || resp.detection?.result || 'Gesture',
                        confidence: resp.detection?.confidence ?? 0,
                        boundingBox: bb ? { x: bb.x, y: bb.y, width: bb.width, height: bb.height } : undefined,
                        model: 'Hand Gesture Recognition',
                        additionalInfo: { processingTime: resp.detection?.processingTime }
                    };
                    if (result.confidence >= confidenceThreshold) {
                        addDetectionResult(result);
                        setDetectionCount(prev => prev + 1);
                        setCurrentDetections([result]);
                    }
                }
            } else if (active && active.key === 'license-plate-recognition') {
                const formData = new FormData();
                formData.append('image', file);
                // Reuse API method to keep consistency
                const resp = await visionAPI.detectLicensePlate(file);
                if (resp?.success) {
                    const bb = resp.detection?.boundingBoxes?.[0];
                    const result = {
                        label: resp.detection?.licensePlateText || resp.detection?.result || 'License Plate',
                        confidence: resp.detection?.confidence ?? 0,
                        boundingBox: bb ? { x: bb.x, y: bb.y, width: bb.width, height: bb.height } : undefined,
    ...
    }, [activeModels, confidenceThreshold, addDetectionResult, lastProcessTime]);
